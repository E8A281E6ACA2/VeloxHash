...
#include "base/api/DashboardHtml.h"
...
static inline bool isDashboardRoute(const HttpData &data)
{
    return data.method == HTTP_GET && (data.url == "/" || data.url == "/index.html" || data.url == "/dashboard");
}


static inline void sendDashboard(uint64_t id)
{
    HttpResponse response(id);
    response.setHeader(HttpData::kContentType, "text/html; charset=utf-8");
    response.setHeader("Cache-Control", "no-store");
    response.setHeader("X-Content-Type-Options", "nosniff");
    response.end(kDashboardHtml, strlen(kDashboardHtml));
}
...
#       endif
    ...
    if (isDashboardRoute(data)) {
        return sendDashboard(data.id());
    }
